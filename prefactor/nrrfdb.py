...
def makePageInfo(nrrset:NrrSetInfo,/) -> NrrPageInfo:
    ...
    regex_latex = f'[{nrrset.regex.replace('*','^*')}]'
    ...
    if len(nrrset.equiv_nrrs) == 0:
        ...
        for nrr in nrrset.equiv_nrrs:
            ...
            # k * [r](n+s)
            info += f'\n\\[[{nrr.regex.replace('*','^*')}]_n=' \
                + (f'{n}\\times' if n != 1 else '') \
                + f'[{r.replace('*','^*')}]_{f'{{n{s:+}}}' if s != 0 else 'n'}' \
                + (f'/{d}' if d != 1 else '') \
                + f'\\]'
...
def _nrrfactorstr(nrrset:NrrSetInfo) -> str:
    ...
    ret = f'\\[[{nrrset.regex.replace('*','^*')}]_n={nrrset.latex}='
    if nrrset.poly_mult.numerator != 1:
        ret += f'{nrrset.poly_mult.numerator}\\times'
    ...
    for k,s,r in nrrset.poly_factors_equiv:
        ret += f'\\left('
        if k.numerator != 1:
            ret += f'{k.numerator}\\times'
        ret += f'[{r.replace('*','^*')}]_{f'{{n{s:+}}}' if s != 0 else 'n'}'
        if k.denominator != 1:
            ret += f'/{k.denominator}'
        ret += f'\\right)'
    if nrrset.poly_mult.denominator != 1:
...
if __name__ == '__main__':
    ...
    if args.python_make_categories:
        ...
        for i in range(1,args.base):
            d = DIGITS[i]
            print(f"dbcat.createCategory('stdkmd-nrr/{args.base}/{d}', is_table = False, "
        ...
        for nrrset in nrrdata:
            ...
            print(f"dbcat.createCategory('stdkmd-nrr/{args.base}/{s[0]}/{s}', "
                  f"is_table = True, title = 'Pattern {s}: {t}', info = '')")
            #for nrr in nrrset.equiv_nrrs:
            #    s = nrr.stdkmd
            #    print(f"dbcat.createCategory('stdkmd-nrr/{s[0]}/{s}', "
    ...
    if args.python_set_descriptions:
        ...
        for nrrset in nrrdata:
            ...
            if len(nrrset.poly_factors) > 1: # factorable
                ...
                print(f"dbcat.setCategoryTitle('stdkmd-nrr/{args.base}/{s[0]}/{s}',"
                    f"{repr(detail.title)})")
                lines = detail.info.splitlines()
                print(f"dbcat.setCategoryInfo('stdkmd-nrr/{args.base}/{s[0]}/{s}',")
        ...
        for i in range(1,args.base):
            d = DIGITS[i]
            print(f"dbcat.setCategoryTitle('stdkmd-nrr/{args.base}/{d}',"
                  f"'Patterns Starting With {d}')")
            if len(per_digit_factors[d]) == 0:
                print(f"dbcat.setCategoryInfo('stdkmd-nrr/{args.base}/{d}',\n"
